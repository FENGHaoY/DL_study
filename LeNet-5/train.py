...
def prepare_data():
    train_data = FashionMNIST('./data',train=True,transform=transforms.Compose([transforms.Resize(size=28), transforms.ToTensor()]),
                          download = True)
    train_data,val_data = Data.random_split(train_data,[round(0.8*len(train_data)), round(0.2*len(train_data))])
    train_dl = Data.DataLoader(train_data,batch_size=128,shuffle=True)
    val_dl = Data.DataLoader(val_data,batch_size=64,shuffle=True)
