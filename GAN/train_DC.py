...
def train():
    ...
    gen_model = DCgen(noise_dim=noise_dim).to(device)# G model
    gen_opt = opt.Adam(params=gen_model.parameters(),lr=lr, betas=(beta_1,beta_2))
    disc_model = DCdisc().to(device)# D model
    ...
    torch.save(gen_model.state_dict(), f'./runs/model_params/DCgen_model_params-{epochs}.pth')
    # 保存判别器模型参数
    torch.save(disc_model.state_dict(), f'./runs/model_params/DCdisc_model_params-{epochs}.pth')
...
def plot_res(gen_list:list, disc_list:list):
    ...
    plt.savefig(f"./runs/DCGAN_Losses-{epochs}")
    plt.show()
