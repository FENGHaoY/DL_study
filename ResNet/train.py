...
from model import ResNet18, Residual # import your model
import copy
import pandas as pd
...
def prepare_data():
    train_data = FashionMNIST('./data',train=True,transform=transforms.Compose([transforms.Resize(size=224), transforms.ToTensor()]),
                          download = True)# change your data
...
def train(model, train_dl, val_dl, epochs):
    ...
    for epoch in range(epochs):
        ...
        for step,(x,y) in enumerate(train_dl):
            ...
            train_running_acc += torch.sum(res == y.detach()).item()#加速操作 把y的计算图分离 同时只保留值 不是Tensor 方便绘图
            train_num += x.size(0)
    ...
    torch.save(best_weights,f'./runs/ResNet18-{epochs}.pth')#修改保存路径
...
if __name__ == '__main__':
    model = ResNet18(Residual)#change your model
